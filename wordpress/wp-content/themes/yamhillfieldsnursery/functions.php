...
add_action( 'wp_enqueue_scripts', function() {   
    ...
    wp_enqueue_style( 'styles', "" . get_template_directory_uri() . '/assets/css/main-styles.css?mod=01122026' );
    wp_enqueue_style( 'print-styles', "" . get_template_directory_uri() . '/assets/css/print-styles.css?mod=01122026' );
        ...
        $itemContainer .= '<div class="woocommerce product-page-products">';
            while( $products->have_posts() ) : $products->the_post(); global $product;  
                $itemContainer .= '<div class="item col-sma-4">';
                    ...
                    $itemContainer .= '</div>';
                    $itemContainer .= '<button class="item__inspect-background" aria-label="inspect item"></button>';
                    ...
                    $itemContainer .= '<div class="item__price">$' . wc_format_decimal( $product->get_price(), 2 ) . 
                            '<a class="item__add-to-cart ajax_add_to_cart add_to_cart_button" href="' . $product->add_to_cart_url() . '" data-product_id="' . get_the_ID() . '" data-product_sku="' . esc_attr( $product->get_sku() ) . '" aria-label="Add ' . the_title_attribute( 'echo=0' ) . ' to cart.">Add to Cart</a>';
                    $itemContainer .= '</div>';
                $itemContainer .= '</div>';       
            endwhile;
        $itemContainer .= '</div>';     
